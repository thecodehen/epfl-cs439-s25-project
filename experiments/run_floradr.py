...
from src.floraDecreasingRanks import FloraAdamDecreasingRanks
...
def main():
    ...
    parser.add_argument('--rankFrom', type=int, default=64)
    parser.add_argument('--rankTo', type=int, default=2)
    ...
    print('FloraDR ', 'lr: ', args.lr, ' rankFrom: ', args.rankFrom, ' rankTo: ', args.rankTo, ' kappa: ', args.kappa, ' seed: ', args.seed, 'Accuracy: ', acc * 100, '%')

    # save the models
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    save_path = pathlib.Path('results') / 'floraDR' / f"lr_{args.lr}-kappa_{args.kappa}-rankFrom_{args.rankFrom}-rankTo_{args.rankTo}-hidden_size_{args.hidden_size}-clip_norm_{args.clip_norm}-seed_{args.seed}-epochs_{args.epochs}-{timestamp}"
