...
def main():
    ...
    train_losses, validation_losses = train_model(model, args.epochs, train_dataset, validation_dataset, loss_fn, optimizer, args.clip_norm, device=device)
    ...
    save_path = pathlib.Path('results') / 'flora' / f"rand_distribution_{args.rand_distribution}-rank_{args.rank}-hidden_size_{args.hidden_size}-clip_norm_{args.clip_norm}-seed_{args.seed}-{timestamp}"
